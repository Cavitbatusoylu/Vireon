...
namespace Vireon.PresentationLayer.Controllers
{
    ...
    public class TransfersController : ControllerBase
    {
        ...
        public IActionResult SendTransfer([FromBody] TransferRequestDto requestDto)
        {
            ...
            var senderOwner = _transactionService.GetAccountById(requestDto.SenderAccountId);
            if (senderOwner == null)
                return NotFound(new { message = "Sender account not found.", mesaj = "Gönderici hesap bulunamadı." });
            if (!IsOwnerOrAdmin(senderOwner.UserId))
                return Forbid();
        ...
        public IActionResult SendTransferByAccount([FromBody] TransferByAccountDto dto)
        {
            try
            {
                ...
                if (!IsOwnerOrAdmin(senderAccount.UserId))
                    return Forbid();
        ...
        public IActionResult Deposit([FromBody] DepositDto dto)
        {
            try
            {
                ...
                var targetAccount = _transactionService.GetAccountByNumber(dto.AccountNumber.Trim().ToUpperInvariant());
                if (targetAccount == null)
                    return NotFound(new { message = "Account not found.", mesaj = "Hesap bulunamadı." });
                if (!IsOwnerOrAdmin(targetAccount.UserId))
                    return Forbid();
        ...
        private bool IsOwnerOrAdmin(int accountOwnerUserId) =>
            User.IsInRole("Admin") ||
            User.FindFirstValue(ClaimTypes.NameIdentifier) == accountOwnerUserId.ToString();
    }
}
