...
function handleRegister(e) {
    ...
    // Geliştirme aşamasında simülasyon
    console.log('Register attempt:', { name, email });
    const btn = e.target.querySelector('button[type=\"submit\"]');
    const originalText = btn.innerHTML;
    btn.innerHTML = currentLang === 'tr' ? 'Kayıt Olunuyor...' : 'Signing up...';

    setTimeout(() => {
        btn.innerHTML = originalText;
        closeRegisterModal();
        alert(currentLang === 'tr' ? 'Kayıt başarılı! Lütfen giriş yapın.' : 'Registration successful! Please sign in.');
        openLoginModal();
    }, 1500);
}
...
function handleForgotPassword(e) {
    ...
    const btn = e.target.querySelector('button[type=\"submit\"]');
    const originalText = btn.innerHTML;
    btn.innerHTML = currentLang === 'tr' ? 'Gönderiliyor...' : 'Sending...';

    setTimeout(() => {
        btn.innerHTML = originalText;
        closeForgotPasswordModal();
        alert(currentLang === 'tr' ? 'Şifre sıfırlama bağlantısı e-postanıza gönderildi.' : 'Password reset link sent to your email.');
    }, 1500);
}
